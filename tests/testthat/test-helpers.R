...
test_that("Default rates must be correct when 'e' is specified", {
  expect_snapshot_output(cat(default_rates(e = 0.5)))
})

test_that("Default rates must be correct when 'e' is unspecified", {
  expect_snapshot_output(cat(default_rates()))
})

test_that("req_e() must produce correct rates", {
  expect_snapshot_output(cat(req_e()))
})
